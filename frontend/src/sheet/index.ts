...
function init_editable_binaries() {
    ...
    checkboxes.forEach((element) => {
        if (
            ...
            element.getAttribute("data-type") === "binary"
        ) {
            element.onchange = () => {
...
function init_editable_tables() {
    ...
    const tables: HTMLTableElement[] = <HTMLTableElement[]>(
        Array.from(document.getElementsByClassName("editableTable"))
    );
...
function init_editable_tables_2() {
    ...
    const tables: HTMLTableElement[] = <HTMLTableElement[]>(
        Array.from(document.getElementsByClassName("editableTable2"))
    );
...
function init_check_progress() {
    ...
    const bars: HTMLSpanElement[] = <HTMLSpanElement[]>(
        Array.from(document.getElementsByClassName("editable_check_progress"))
    );
    bars.forEach((bar) => {
