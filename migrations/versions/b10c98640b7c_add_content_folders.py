...
def upgrade():
    ...
    op.create_table('folder',
                    ...
                    sa.ForeignKeyConstraint(['owner_id'], ['user_profile.id'], name=op.f(
                        'fk_folder_owner_id_user_profile')),
                    sa.ForeignKeyConstraint(['parent_id'], ['folder.id'], name=op.f(
                        'fk_folder_parent_id_folder')),
    ...
    with op.batch_alter_table('campaign', schema=None) as batch_op:
        ...
        batch_op.create_foreign_key(batch_op.f(
            'fk_campaign_folder_id_folder'), 'folder', ['folder_id'], ['id'])

    with op.batch_alter_table('charactersheet', schema=None) as batch_op:
        batch_op.add_column(sa.Column('folder_id', GUID(), nullable=True))
        batch_op.create_foreign_key(batch_op.f(
            'fk_charactersheet_folder_id_folder'), 'folder', ['folder_id'], ['id'])
...
def downgrade():
    ...
    with op.batch_alter_table('charactersheet', schema=None) as batch_op:
        batch_op.drop_constraint(batch_op.f(
            'fk_charactersheet_folder_id_folder'), type_='foreignkey')
        batch_op.drop_column('folder_id')

    with op.batch_alter_table('campaign', schema=None) as batch_op:
        batch_op.drop_constraint(batch_op.f(
            'fk_campaign_folder_id_folder'), type_='foreignkey')
        batch_op.drop_column('folder_id')
